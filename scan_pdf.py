...
with open(r"c:\Users\dxdel\Extra Activities\iis-servicemanagment\pdf_scan.txt", "w", encoding="utf-8") as f:
    f.write("\n".join(lines))

# pages with substantial text
substantial = []
for i in range(doc.page_count):
    t = doc[i].get_text().strip()
    if len(t) > 100:
...
with open(r"c:\Users\dxdel\Extra Activities\iis-servicemanagment\pdf_text_pages.txt", "w", encoding="utf-8") as f:
